...
import type { ResourceMutation } from "@/components/shared/configuration/resource-data.types"
import {
  configurationUi,
  ...
  return (
    <Dialog
      ...
      onOpenChange={(nextOpen) => {
        ...
        else if (!pending) closeDialog()
      }}
    >
      <DialogContent
        className={resource === "roles" ? "sm:max-w-2xl" : undefined}
      >
        <DialogHeader>
          <DialogTitle>
            {entity ? "Modifier" : "Ajouter"} {ui.singular}
          </DialogTitle>
          <DialogDescription>
            {entity
